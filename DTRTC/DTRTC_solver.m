...
%% update (X{u},Y{u})
  oldTC=TC;
  gamma=1*norm(TC2{1}(known)-data)/norm(TTC(known)-data);
  if k<=1
      %--------- X---------------------
      for n = 1:floor((n3{1}+1)/2)
          X{1,n}=C{1}(:,:,n)*Y{1,n}'*pinv(Y{1,n}*Y{1,n}');
          Xsq{1,n} = X{1,n}'*X{1,n};
          C{1}(:,:,n)=X{1,n}*Y{1,n};
      end
      for n = floor((n3{1}+1)/2)+1:n3{1}
          C{1}(:,:,n)=conj(C{1}(:,:,n3{1}-n+2));
      end
      TC2{1}=ifft(C{1},[],3);
      TC=(TC2{1}+gamma*Fold(Unfold(TC2{2},Nway{2},1),Nway{1},3))/(1+gamma);
      ...
      for n = 1:floor((n3{2}+1)/2)
          X{2,n}=C{2}(:,:,n)*Y{2,n}'*pinv(Y{2,n}*Y{2,n}');
          Xsq{2,n} = X{2,n}'*X{2,n};
          C{2}(:,:,n)=X{2,n}*Y{2,n};
      end
      for n = floor((n3{2}+1)/2)+1:n3{2}
